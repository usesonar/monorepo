...
- Requests are flat: `seed`, `ttl`, `person`, `company`, and the matching `research` or `deepResearch` question map.
- Use `question<Answer>(prompt)` for typed custom answers in API create operations, Effect, React, and static Eve. Finite interfaces and finite type aliases produce exact required fields. Question maps annotated as `Readonly<Record<string, string>>` remain accepted, but arbitrary custom reads are `Field<JSONValue> | undefined`; broad catalog arrays also make reads optional while preserving catalog value types. Reject callable or constructable maps and numeric or symbol key hybrids. Do not add operation-level answer maps; raw API retrieval and dynamic Eve are the only surfaces that may accept explicit maps because neither owns a fixed question config at the call site.
- Results list `person`, then `company`, then custom answers as top-level fields. Never add a nested custom-result namespace.
