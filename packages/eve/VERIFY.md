...
Both factories use `factory(config, options?)`. Research options are limited to `layer` and `description`; deep options add only `execution: "background"`. Config owns `ttl`, `person`, `company`, the matching custom question map, and optional `dynamic`. Callers cannot spread arbitrary `defineTool` fields or replace approval, schemas, execute, output projection, or background task behavior.
...
Static config puts the exact configured `person`, `company`, and custom question map into developer-owned configuration. The model input contains only a seed with optional `linkedinURL`, `fullName`, `xURL`, `email`, `domain`, and JSON `context`, and it accepts the seed only when at least one prerequisite is present: `linkedinURL`, `fullName` plus `xURL`, or `fullName` plus `email`.
...
With `dynamic: true`, the model input owns `person`, `company`, and the matching `research` or `deepResearch` question map in addition to the seed. `ttl`, `execution`, and appended description remain developer-owned and never appear in model input. Dynamic input uses the correct tier catalog, lists `person` before `company`, rejects wrong-tier fields, and requires at least one requested built-in or custom field.

Custom keys are camelCase identifiers and reject the reserved keys `ttl`, `person`, `company`, `research`, and `deepResearch`. Custom questions are non-empty strings. Static and dynamic validation apply the same rules.

The Effect request preserves insertion order as `seed`, `ttl`, `person`, `company`, then `research` or `deepResearch`. PASS only if Standard Schema validation and request-routing assertions cover both tiers, including the distinction that config/input keeps the question map while results flatten only its custom answer keys.

### EVE-05 — Generated descriptions and schemas

Descriptions are deterministic and list the exact selected fields in configuration order: `person` first, then `company`, then top-level custom keys. They state the tier's expected latency. Dynamic descriptions list the selectable catalog and model-supplied question behavior. A caller description is appended after the generated description without replacing it.

Input and output remain actual Effect Schema values for which `Schema.isSchema` returns `true`. Eve `0.45.1` can consume the same values through Standard Schema and compile them to JSON Schema. The output schema is the full `{ status, data }` snapshot, whose `data` contains nested person fields, nested company fields, then top-level custom keys. Static `question<Answer>(prompt)` values infer exact required custom fields with the branded answer type, while unbranded prompt strings infer required `JSONValue` fields. Finite static interfaces and finite static type aliases produce the same exact required fields. A broad static `Readonly<Record<string, string>>` annotation exposes arbitrary custom reads as `Field<JSONValue> | undefined`, and broad catalog arrays expose optional fields with their catalog value types. Static maps reject unions, optional keys, callable or constructable behavior, and numeric or symbol key hybrids. Dynamic built-ins remain partial, and explicit dynamic `<Answers>` custom fields remain optional because the model owns and may omit or replace question keys. Those explicit maps require finite, non-union interfaces or type aliases with required lower-camel keys and reject the same callable, constructable, numeric, and symbol forms. The locked TypeScript 5 semantic probe proves that default dynamic callback data is not `never`, preserves those optional dynamic fields, and retains static branded inference.
...
`toModelOutput` projects from the full snapshot's `data` into Eve JSON model output whose `value` preserves the same result nesting and order: resolved person fields, resolved company fields, then resolved top-level custom keys. Every included leaf has own-key order `value`, then `confidence`. Unresolved leaves and empty containers are omitted.

Static factories treat their validated configured custom keys as authoritative regardless of spelling, so configured answers named `jobFit`, `provider`, `cache`, `jobId`, or `hash` survive projection while unconfigured custom leaves are omitted. Dynamic factories project every schema-valid top-level custom answer they receive; they do not infer the model's selected question keys again inside `toModelOutput`, because the Effect request and protocol boundaries already prevent unrequested leaves from reaching a valid snapshot. Projection omits the snapshot envelope and any envelope hash, plus per-field `status`, `sources`, and timestamps such as `resolvedAt`. It never mutates the full snapshot, so the Eve `action.result`, channels, hooks, and clients retain the full value.
